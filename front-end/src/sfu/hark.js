...
export default function hark(stream, options) { // 현재 js에는 클래스 개념이 있지만, 이전에는 js에 클래스 개념이 없어서, 함수에서 자가 인스턴스를 생성해서 사용하는 방식을 사용함
    // 오디오 통신 컨택스트 객체
    ...
    harker.events = {};
    // 함수 자가 메소드 정의 (websocket에서 사용하는 on 메서드가 아니라, 그냥 harker에서 자체적으로 생성한 on 메서드)
    harker.on = function (event, callback) {
        harker.events[event] = callback;
    };

    // 함수 자가 메소드 정의 (Node.js의 .on, .emit 메서드가 아닌 함수 속 자가 메서드를 생성한 것.)
    // 아마 이벤트 리스너 사용을 위한 구조인거 같은데... -> 맞음 트리거 역할. 이벤트 발생 시 메시지로 안받아도 발생하게끔 되는듯
    ...
    // console.log("hark.js line 43 window.audioContext00 : ", window.audioContext00);
    ...
    // console.log("hark line 58 sourceNode : ", sourceNode);
